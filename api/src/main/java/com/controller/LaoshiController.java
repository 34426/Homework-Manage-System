...
public class LaoshiController {
	...
	public R login(String username, String password, String captcha, HttpServletRequest request) {
		LaoshiEntity user = laoshiService.selectOne(new EntityWrapper<LaoshiEntity>().eq("gonghao", username));
		if(user==null || !user.getMima().equals(password)) {
			return R.error("账号或密码不正确");
		}

		request.getSession().setAttribute("userId", user.getId());
		request.getSession().setAttribute("role", "laoshi");
		request.getSession().setAttribute("tableName", "laoshi");
		request.getSession().setAttribute("username", user.getGonghao());
    ...
    public R register(@RequestBody LaoshiEntity laoshi){
    	...
    	LaoshiEntity user = laoshiService.selectOne(new EntityWrapper<LaoshiEntity>().eq("gonghao", laoshi.getGonghao()));
		if(user!=null) {
